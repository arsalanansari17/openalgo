...
def verify_api_key_no_cache(provided_api_key):
    ...
    thread (a broker streaming adapter, which per CLAUDE.md's eventlet
    invariant runs unpatched in its own process) while a Flask greenlet
    holds the same lock causes greenlet.error. Use this variant from any
    non-greenlet context (see GitHub issue #1421).
    """
...
def get_broker_name_no_cache(user_id):
    ...
    See verify_api_key_no_cache for why this is needed from a real OS thread.
    """
    if not user_id:
        return None
    try:
