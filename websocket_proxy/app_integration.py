...
# Bug 5 (#1421): logging.Handler.lock is monkey-patched by eventlet to an
# eventlet Semaphore. Any logger.xxx() call from the asyncio WS-proxy OS
# thread contends this lock cross-thread → greenlet.error: Cannot switch to
# a different thread. Replace with real OS threading.RLock on all current
# and future handlers so logging is safe from any OS thread.
if "eventlet" in sys.modules:
    import logging as _logging
    _orig_rlock = _original_threading.RLock
    _logging.Handler.createLock = lambda self: _orig_rlock()
    def _patch_logging_handler_locks():
        all_loggers = [_logging.root] + [
            lgr for lgr in _logging.Logger.manager.loggerDict.values()
            if isinstance(lgr, _logging.Logger)
        ]
        for lgr in all_loggers:
            for h in lgr.handlers:
                h.lock = _orig_rlock()
    _patch_logging_handler_locks()
    del _patch_logging_handler_locks
